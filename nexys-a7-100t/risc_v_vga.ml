...
include struct
  ...
  module Vga = Hardcaml_hobby_boards_kernel.Vga
...
module Vga_config = struct
  let input_width = 64
  let input_height = 48
  let output_width = Vga.Spec.t640x480_60hz.horizontal_timing.active
  let output_height = Vga.Spec.t640x480_60hz.vertical_timing.active
  let start_address = 16384
  let vga_spec = Vga.Spec.t640x480_60hz
  let vga_clock_div = 2
end
...
module Cpu =
  Risc_v.System_with_bram.Make
    ...
    (struct
      ...
      let ethernet_start_address = 2048
      let frame_buffer_bytes = Vga_config.input_width * Vga_config.input_height * 12 / 8
    end)
    (struct
      let num_harts = 1
...
let create () =
  ...
  let ethernet_inputs = Nexys.Ethernet.create board in
  ...
  let cpu =
    Cpu.create
      ...
      ; clear = clear_50
      ; uart_rx = uart.rxd
      ; eth_crsdv = ethernet_inputs.crsdv
      ; eth_rxerr = ethernet_inputs.rxerr
      ; eth_rxd = ethernet_inputs.rxd
      }
  in
  Nexys.Ethernet.complete
    board
    { Nexys.Ethernet.O.txen = cpu.ethernet_txen
    ; txd = cpu.ethernet_txd
    ; rstn = clocking.reset_n
    ; refclk = clock_50
    };
  Nexys.Uart.complete board { Nexys.Uart.O.rts = gnd; txd = cpu.uart_tx };
  Nexys.Vga.complete
    board
    { Nexys.Vga.O.red = cpu.vga_red
    ; green = cpu.vga_green
    ; blue = cpu.vga_blue
    ; horizontal_sync = cpu.vga_hsync
    ; vertical_sync = cpu.vga_vsync
    };
  board
;;
